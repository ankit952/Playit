...
public class FXMLDocumentController implements Initializable {
    ...
    @FXML
    private static final double min = 0.4;
    @FXML
    ...
    private void handleButtonAction(ActionEvent event) {
        ...
        FileChooser.ExtensionFilter fl= new FileChooser.ExtensionFilter("Open File ","*.mp4","*.mp3");
            ...
            if(path != null){
                ...
                med.totalDurationProperty().addListener((obs, oldDuration, newDuration) -> sSlider.setMax(newDuration.toSeconds()));
                sSlider.valueChangingProperty().addListener((obs, wasChanging, isChanging) -> {
                 if(!isChanging)   {
                     med.seek(Duration.seconds(sSlider.getValue()));
                    }
                });
                sSlider.valueProperty().addListener((obs, oldValue, newValue) -> {
                if (! sSlider.isValueChanging()) {
                    double currentTime = med.getCurrentTime().toSeconds();
                    if (Math.abs(currentTime - newValue.doubleValue()) > min) {
                            med.seek(Duration.seconds(newValue.doubleValue()));
                        }
                    }
                });
                med.currentTimeProperty().addListener((obs, oldTime, newTime) -> {
                    if (! sSlider.isValueChanging()) {
                        sSlider.setValue(newTime.toSeconds());
                    }
                });
